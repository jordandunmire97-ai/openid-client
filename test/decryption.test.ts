...
test('ECDH-ES JWE decryption accepts a P-256 private key', async (t) => {
  const issuer = new URL('https://as.example.com')
  const signingKeyPair = await client.randomDPoPKeyPair('ES256')
  const ecdhKeyPair = await crypto.subtle.generateKey(
    { name: 'ECDH', namedCurve: 'P-256' },
  ...
  client.enableDecryptingResponses(config, undefined, ecdhKeyPair.privateKey)
  config[client.customFetch] = async () =>
    new Response(
      JSON.stringify({
        access_token: 'test-access-token',
